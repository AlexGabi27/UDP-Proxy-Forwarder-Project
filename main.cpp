...
char ips[64][32] = {
    "0", "89.40.233.43", "51.38.121.178", "188.212.101.191"
...
vector<clients_ev> clients_vector;
...
struct clientinfo
{
    ...
    void Setup(char *ip, int port)
    {
        ...
        fwdserver.sin_addr.s_addr = inet_addr(ip);
        fwdserver.sin_port = htons(port);
    ...
    void HostnameChange(char type)
    {
        ...
        if(type == 'I')
        {
            char static_buff[]     = "\x00\x63\x73\x74\x72\x69\x6B\x65\x00\x43\x6F\x75\x6E\x74\x65\x72\x2D\x53\x74\x72\x69\x6B\x65\x00\x0A\x00\x20\x20\x01\x64\x6C\x00\x01\x31\x2E\x31\x2E\x32\x2E\x37\x2F\x53\x74\x64\x69\x6F\x00";
            char static_buff2[]    = "\x00\x63\x73\x74\x72\x69\x6B\x65\x00\x43\x6F\x75\x6E\x74\x65\x72\x2D\x53\x74\x72\x69\x6B\x65\x00\x0A\x00\x02\x20\x01\x64\x6C\x00\x01\x31\x2E\x31\x2E\x32\x2E\x37\x2F\x53\x74\x64\x69\x6F\x00";
            ...
            hostname_id = random(0, hostname_len-1, porta);
            ...
            int client_in_vector = -1;
            if(clients_vector.size() > 0)
            {
                for(int i = 0; i < clients_vector.size(); i++)
                {
                    if(clients_vector[i].client.sin_addr.s_addr == client.sin_addr.s_addr)
                    {
                        client_in_vector = i;
                        break;
                    }
            ...
            if(client_in_vector == -1)
            {
                ...
                clients_vector.push_back(cclient);
            }
            else
            {
                concat(static_buff2, static_buff_len);
                clients_vector[client_in_vector].counts++;
                if(clients_vector[client_in_vector].counts >= 2)
                {
                    clients_vector.erase(clients_vector.begin()+client_in_vector);
                    clients_vector[client_in_vector].counts = 0;
                }
            }
...
void ReadIPS()
{
    while(1)
    {
        fstream ipsf("config//ips.txt");

        string line;
        ips_len = 1;
        while(getline(ipsf, line))
        {
            // ips[ips_len++] = line.c_str();
            strcpy(ips[ips_len], line.c_str());
            getline(ipsf, line);
            port[ips_len++] = stoi(line);
        }
        sleep(300);
    }
}
...
int main(int argvn, char *argv[])
{
    ...
    if (!SteamGameServer_Init(INADDR_ANY, 26900, porta, 0xFFFFu, eServerModeAuthenticationAndSecure, "1.1.2.7"))
    {
        cout << "[Steam] Could not initialize Steam.\n";
    }

    thread(SteamCallbacksThread).detach();
    SetServerParams();
    ...
    while(1)
    {
        ...
        if ((bufferlen = recvfrom(serversock, buffer, sizeof(buffer), 0, (struct sockaddr *)&client, &clientlen)))
        {
            ...
            if (CheckIfConnectPacket((char *)buffer))
            {
                ...
                char name[64];
                char *name_p;
                if ((name_p = strstr((char *)buffer, "name")) && ticket_mode == false)
                {
                    int i = 0, k = 0;
                    name[k++] = 0x27;
                    while (name_p[i++] != '\\')
                    ...
                    char thing[32] = "python3 insert.py ";
                    char space[2] = " ";
                    char tt[2] = ":";
                    concat(command, thing);
                    concat(command, name);

                    concat(command, inet_ntoa(client.sin_addr));
                    concat(command, space);
                    if(currip > 3) currip = 0;
                    concat(command, ips[currip]);
                    concat(command, tt);

                    std::string port_p = std::to_string(27015);

                    concat(command, (char *)port_p.c_str());
            ...
            if (clients[clientsid].client.sin_addr.s_addr != client.sin_addr.s_addr)
            {
                ...
                if(strstr((char *)buffer, "getchallenge steam"))
                {
                    if(currip > 3)
                    {
                        currip = 1;
                    }
                    cout << ips[currip] << ":" << ports[currip] <<  ", " << currip << endl;
                    clients[clientsid].Setup(ips[currip], ports[currip++]);
                }
                else
                {
                    clients[clientsid].Setup(ips[1], ports[1]);
                }
